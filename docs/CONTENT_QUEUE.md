...
S01 draft narration: “We're using AI to build an actual game. This is Catmurai: she already fights, levels up, and takes on bosses. Now we're turning those systems into a game worth playing. Next challenge: give her an ultimate.”

L01 story: cold-open combat (0:00); goal and current state (0:15); the existing systems (0:45); a concrete blocker and fix (1:45); failed and successful tests (3:00); honest result (4:30); next feature (5:15). Timing is an editing target, not fabricated footage. Do not claim two agents collaborated until Claude actually contributes.

Capture folder: content/raw/YYYY-MM-DD_feature-name/. Keep before/final clips, screenshots and notes. Current recorder exports silent game canvas only; use separate narration/SFX and a full-screen capture workflow if HUD/dialogue is needed. Save WebM from the visible Save clip link, then file it in the capture folder. Raw capture is not a published Short.
